...
export async function obterConteudoArquivo(auth: any, fileId: string): Promise<string> {
  const drive = google.drive({ version: 'v3', auth })
...
export async function atualizarArquivoNaPasta(auth: any, fileId: string, conteudo: string, mime='application/json'){
  const drive = google.drive({ version: 'v3', auth })
  const stream = Readable.from([conteudo])
...
export async function moverParaLixeira(auth: any, fileId: string){
  const drive = google.drive({ version: 'v3', auth })
