...
export async function GET(req: NextRequest){
  ...
  try {
    ...
    const mensagem = e?.response?.data?.error_description || e?.response?.data?.error || e?.message || 'erro desconhecido'
    return NextResponse.json({ etapa: 'callback-google', erro: mensagem }, { status: 500 })
  }
}
